...
func New(b *service.BusinessService, a *service.AuthService, s *service.SubscriptionService) *Server {
	e := echo.New()
	srv := &Server{Echo: e, Businesses: b, Auth: a, Subscriptions: s}
	srv.routes()
	return srv
}
func (s *Server) routes() {
	s.Echo.GET("/health", func(c echo.Context) error { return c.JSON(http.StatusOK, map[string]any{"status": "ok"}) })
...
func (s *Server) getBusiness(c echo.Context) error {
	b, err := s.Businesses.GetBusiness(c.Param("id"))
...
func (s *Server) createReview(c echo.Context) error {
	...
	r, err := s.Businesses.CreateReview(c.Param("id"), req.UserID, req.Rating, req.Body)
...
func (s *Server) webhook(c echo.Context) error {
	...
	if _, err := c.Request().Body.Read(body); err != nil && c.Request().ContentLength > 0 {
