...
func (s *Server) bindJSON(c echo.Context, v any) error {
...
func (s *Server) createReview(c echo.Context) error {
	...
	if err := s.bindJSON(c, &req); err != nil {
		if isBodyTooLarge(err) {
			return c.JSON(413, map[string]string{"error": "request body too large"})
		}
...
func (s *Server) initializeSubscription(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(502, map[string]string{"error": "payment provider unavailable"})
	}
	return c.JSON(200, resp)
}
func (s *Server) webhook(c echo.Context) error {
	body, err := io.ReadAll(http.MaxBytesReader(c.Response(), c.Request().Body, maxBodyBytes))
	if err != nil {
	...
	if err := s.Subscriptions.HandleWebhook(c.Request().Context(), body, c.Request().Header.Get("x-paystack-signature")); err != nil {
		...
		return c.JSON(400, map[string]string{"error": "invalid webhook payload"})
	}
